...
Непересекающиеся бренды        & \textendash & \textendash & \textendash & \textendash & \pos{\checkmark} \\
...
\textbf{проверку на непересекающихся товарных кодах (code-disjoint)} и \textbf{гибридную архитектуру} одновременно.
\end{frame}
...
\begin{enumerate}
    ...
    \item Pre-registered протокол оценки на выборке с непересекающимися товарными кодами (code-disjoint).
...
\begin{itemize}
    \fontsize{10pt}{13pt}\selectfont
    \setlength{\itemsep}{0.15em}
    \item $\mathrm{Acc}(\pi)$ на проверке с непересекающимися товарными кодами (code-disjoint).
    \item $C(\pi)$ — стоимость относительно прямого вызова большой языковой модели.
    \item Покрытие каскадом без обращения к языковой модели.
\end{itemize}
...
2. Требования и качество    & 6 ФР, 5 НФР, 3 А; интегральный критерий $Q$ — все цели достигнуты. \\
3. Каскад и маршрутизация   & 4 слоя, 21 пара по 3 категориям (8/6/7), калиброванные пороги. \\
4. Реализация               & \texttt{docker-compose} + REST + веб-интерфейс; $\sim$15\,тыс. строк Python. \\
5. Предрегистрируемая оценка & $n{=}3257$: точность каскада \pos{94{,}8\,\%}\,/\,\pos{0{,}899} макро-F1, производственная цепочка \pos{91{,}1\,\%}, \pos{14$\times$} дешевле. \\
