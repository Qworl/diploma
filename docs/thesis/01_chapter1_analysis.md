...
![Процесс работы контент-менеджера: (а) текущий, (б) целевой с интеграцией гибридного каскада обогащения](figures/fig_1_1_cm_process.png)

Рисунок 1.2 — Процесс работы контент-менеджера: (а) текущий, (б) целевой с интеграцией гибридного каскада обогащения
...
Задача восполнения пропущенных атрибутов рассматривается как частный случай извлечения признаков из неструктурированного текста (information extraction, IE). Систематизация методов извлечения информации из текста, включая выделение именованных сущностей и фактов, приведена в обзоре Л. М. Ермаковой [51]; гибридная природа практических решений по выделению ключевых слов и признаков из русскоязычных текстов отмечена в работе А. С. Ванюшкина и Л. А. Гращенко [52]. Подходы делятся на три группы — формальные правила, классические статистические модели и нейросетевые модели на трансформерах — и их сочетание образует основу каскадных архитектур.
...
Принципиальная сложность задачи — отсутствие достаточной ручной разметки; постановка известна как weak supervision, инструментарий систематизирован A. Ratner и соавторами «Snorkel» [26]. Подходы к снижению стоимости разметки через активное обучение и слабый надзор систематизированы в обзоре B. Settles [46]. Пригодным источником слабого эталона выступает Open Food Facts [13] — открытая база >4,5 млн пищевых продуктов с пользовательскими тегами. Эмпирический анализ устойчивости алгоритмов классификации к зашумлению обучающей разметки выполнен В. А. Дюком [48] и показал, что классические методы (KNN, SVM) сохраняют приемлемую точность при умеренной доле ошибок разметки, что обосновывает применимость серебряной разметки.

Для корректной работы каскада выходы классификаторов должны быть откалиброваны: масштабирование Платта [27], изотоническая регрессия [28]; современный анализ ECE для глубоких моделей — C. Guo и соавторы [29]. Систематизация критериев качества классификаторов (включая ROC-анализ и выбор порога вероятности) дана в работах Ю. С. Шуниной и соавторов [49] и А. А. Ковалева и соавторов [50]. Оценка точности при ограниченной выборке — кросс-валидация и бутстрэп [30]; интервальная оценка пропорций — формула Вильсона [31].
