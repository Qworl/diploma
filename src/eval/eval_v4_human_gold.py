"""Eval cascade against HUMAN-labeled gold (manual_eval_per_product).

Splits by clean (not in ML train) vs leak. The honest "non-circular" headline
is the CLEAN subset accuracy — measures how the cascade performs against
real human labels on codes the model never saw at training time.

For pasta (only 16% leak), this gives a near-clean non-circular number.
"""
...
def eval_cat(cat, manual_df, off_dir, leak_codes):
    print(f'\n=== {cat.upper()} ===')
    g = manual_df[manual_df.category == cat].copy()
    ...
    for attr in sorted(merged.attr.unique()):
        ...
        print(f'  {attr:22s}  {c_cov:>7d}  {c_acc:>8.1f}%  {l_cov:>7d}  {l_acc:>8.1f}%')
    ...
    sub_clean_all = merged[(merged.in_scope == True) & (~merged.is_leak)]
    sub_leak_all = merged[(merged.in_scope == True) & (merged.is_leak)]
    c_cov, c_cor, _ = _stats(sub_clean_all)
    l_cov, l_cor, _ = _stats(sub_leak_all)
    print(f'  {"OVERALL":22s}  {c_cov:>7d}  {100*c_cor/max(c_cov,1):>8.1f}%  {l_cov:>7d}  {100*l_cor/max(l_cov,1):>8.1f}%')
    return rows
...
def main():
    ...
    all_rows = []
    for cat in ['pasta', 'chocolate', 'cheeses']:
        all_rows.extend(eval_cat(cat, manual, off_dir, leak_per_cat[cat]))

    df = pd.DataFrame(all_rows)
    out = PROJECT_ROOT / 'datasets/processed/v4_eval_human_gold.parquet'
    df.to_parquet(out, index=False)
    c_cov = df.clean_n.sum(); c_cor = df.clean_correct.sum()
    l_cov = df.leak_n.sum(); l_cor = df.leak_correct.sum()
    print(f'\n=== GRAND (HUMAN gold) ===')
    print(f'  CLEAN (non-circular):  {c_cor}/{c_cov} ({100*c_cor/max(c_cov,1):.1f}%)')
